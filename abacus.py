...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if not is_vercel:
            # 启动后台任务
            app.state.token_refresh_task = asyncio.create_task(session_token_manager.periodic_token_refresh())
            app.state.cleanup_task = asyncio.create_task(cleanup_long_text_sessions())
            log_info("已启动后台任务")
        
        # 记录状态信息
        log_info(f"服务API配置: BASE_URL={BASE_URL}")
        log_info(f"会话配置: MAX_SESSIONS={MAX_SESSIONS}, SESSION_TIMEOUT={SESSION_TIMEOUT}秒")
        log_info(f"并发请求限制: MAX_CONCURRENT_REQUESTS={MAX_CONCURRENT_REQUESTS}")
        log_info(f"HTTP连接池配置: CONNECTIONS={HTTP_POOL_CONNECTIONS}, KEEPALIVE={HTTP_MAX_KEEPALIVE_CONNECTIONS}")
        ...
        # 在非Vercel环境下关闭后台任务
        if not is_vercel and hasattr(app.state, "token_refresh_task"):
            # 取消后台任务
